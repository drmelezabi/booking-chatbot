...
const preparationHumanType = async (inp: string) => {
  try {
    ...
    const theeAfterTomorrow = threeDaysAfterTomorrow.toLocaleDateString(
      "en-US",
      {
        weekday: "short",
      }
    ...
    const letterBeforeNumberArabic =
      /(?<=\p{L})(?=\p{N})|(?<=\p{N})(?=\p{L})/gu;
    ...
    throw ErrorHandler(error, "preparationHumanType");
  }
};
...
const prepareBookingMessage = async (str: string): Promise<TimeDetails> => {
  try {
    ...
    throw ErrorHandler(error, "prepareBookingMessage");
  }
};
